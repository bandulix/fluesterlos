...
import { getHostSettings, readMultipartFile } from "./invoiceHelpers.js";

type EventRow = { id: string; code: string; title: string; starts_at: Date; ends_at: Date };

export function registerGuestInvoiceRoutes(
  app: FastifyInstance,
  deps: {
  ...
  app.get("/api/events/:code/invoice", async (req, reply) => {
    ...
    if (settings.promptpay_id) {
      try {
        promptpayPayload = promptPayPayload(settings.promptpay_id, total);
      } catch {
    ...
    return {
      ...
      invoice: {
        ...
        paidAt: data.invoice.paid_at
          ? new Date(data.invoice.paid_at).toISOString()
          : null,
  ...
  app.post("/api/events/:code/invoice/payslip", async (req, reply) => {
    ...
    await pool.query(
      `UPDATE invoices SET payslip_path = $1, status = 'payslip_uploaded'
