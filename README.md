...
Copy env first (both paths below):

```bash
cp .env.example .env
# set a strong BOOTSTRAP_TOKEN (used only for the first owner register)
```
...
- `ghcr.io/bandulix/fluesterlos-api:latest` (also tagged with commit SHA)
- `ghcr.io/bandulix/fluesterlos-web:latest`

Public packages are preferred (no login). If the packages are private:

```bash
# GitHub PAT or token with read:packages
echo $GITHUB_TOKEN | docker login ghcr.io -u YOUR_GITHUB_USERNAME --password-stdin
```

Pin a specific build with `FLUESTERLOS_TAG` (SHA tag from the workflow, e.g. `abc1234`):

```bash
FLUESTERLOS_TAG=abc1234 docker compose -f docker-compose.ghcr.yml up
```

### After the stack is up

1. Open **Host UI** at http://localhost:8080/host
...
2. Create event + items, share QR / join link
3. Guest joins at `/e/<code>` (name + email only), then bids at `/e/<code>/bid`
4. Public stats: `/e/<code>/stats` — Engager big screen: `/e/<code>/engager`
...
- Prebuilt images: GHCR (`docker-compose.ghcr.yml`); CI builds/pushes on `main`, tags, and `workflow_dispatch`
